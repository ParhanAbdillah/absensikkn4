...
<div class="space-y-6" x-data="{ openAddModal: false, openEditModal: false, selectedReceipt: null, editData: { id: '', date: '', type: 'income', category: 'Kas Anggota', amount: '', description: '', url: '' }, openEdit(item, url) { this.editData = { id: item.id, date: (item.date || '').substring(0, 10), type: item.type, category: item.category, amount: item.amount, description: item.description, url: url }; this.openEditModal = true; } }">
    ...
    <div class="bg-white rounded-2xl border border-slate-150 shadow-sm overflow-hidden">
        <div class="p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-100">
            <div>
                <h1 class="text-md font-bold text-slate-800 tracking-tight flex items-center gap-2">
                    @if (Auth::user()->avatar && file_exists(public_path(Auth::user()->avatar)))
                        <img src="{{ asset(Auth::user()->avatar) }}" alt="{{ Auth::user()->name }}" class="w-10 h-10 rounded-full object-cover border-2 border-emerald-500 shadow-sm flex-shrink-0">
                    @else
                        <div class="w-10 h-10 rounded-full bg-emerald-600 text-white flex items-center justify-center font-extrabold text-sm shadow-sm flex-shrink-0">
                            {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                        </div>
                    @endif
                    Welcome {{ Auth::user()->name }}!! Princess Keuangan Kita 👸💸
                </h1>
                <p class="text-xs text-slate-500 mt-1">Transparansi keuangan KKN Kelompok 4 Desa Sirnaraja</p>
            </div>
