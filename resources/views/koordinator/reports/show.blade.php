<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div class="w-8 h-8 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold text-sm flex-shrink-0">
                {{ strtoupper(substr($user->name, 0, 1)) }}
            </div>
            <div>
                <h2 class="font-extrabold text-xl text-slate-800 tracking-tight leading-tight">
                    {{ $user->name }}
                </h2>
                <p class="text-xs text-slate-500">{{ $user->divisi ?: ($user->class ?: 'Anggota KKN') }}</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-5 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h3 class="font-bold text-slate-800 text-lg">Laporan Kegiatan</h3>
                    ...
                    <div>
                        <form method="GET" action="{{ url()->current() }}">
                            <select name="status" onchange="this.form.submit()"
                                class="text-xs font-semibold bg-slate-50 border border-slate-200 text-slate-700 rounded-xl px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    <table class="w-full text-sm text-left">
                        <thead class="text-xs text-slate-500 bg-slate-50 uppercase font-bold tracking-wider">
                            <tr>
                                ...
                                <th class="px-6 py-4 min-w-[200px]">Kegiatan</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            @php
                                $isNearDeadline = $report->status !== 'Done'
                                    && $report->deadline >= now()
                                    && $report->deadline->diffInDays(now()) <= 3;
                                $isOverdue = $report->status !== 'Done' && $report->deadline < now();
                                $bgClass = $report->status === 'Done'
                                    ? 'bg-emerald-50/30'
                                    : ($isOverdue || $isNearDeadline ? 'bg-red-50/50' : 'hover:bg-slate-50/50');
                            @endphp
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-slate-500">
                                    @if(request('status'))
                                        Tidak ada laporan dengan status <strong>{{ request('status') }}</strong> untuk anggota ini.
                                    @else
                                        Anggota ini belum memiliki laporan kegiatan.
                                    @endif
                                </td>
                            </tr>
                            @endforelse
                ...
                <div class="px-5 py-3 border-t border-slate-100 bg-slate-50 text-xs text-slate-500 flex gap-4">
                    <span>Total: <strong class="text-slate-700">{{ $reports->count() }}</strong> laporan</span>
                    <span>Done: <strong class="text-emerald-700">{{ $reports->where('status','Done')->count() }}</strong></span>
                    <span>In Progress: <strong class="text-amber-700">{{ $reports->where('status','In Progress')->count() }}</strong></span>
                    <span>To Do: <strong class="text-blue-700">{{ $reports->where('status','To Do')->count() }}</strong></span>
                </div>
                @endif
            </div>
